...
export const inngest = new Inngest({ id: "grocery-deliveries" });

// Your new function: low stock alert to admin email
const checkLowStock = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run('send-low-stock-email', async () => {
        ...
        await sendEmail({
            // 'join' so it can join multiple email. you get 'product' below from productId
            to: adminEmails.join(','),
...
// monthly offer email (first of every month-payday)
const sendMonthlyOffers = inngest.createFunction({id: 'send-monthly-offers', name: 'monthly-payday-offers', triggers:[cron('0 10 1 * * ')]}, 
    async({step})=>{
        const {deals,users} = await step.run('fetch-deals-and-users', async () => {
            //get top discounted product at featured deals
            const products = await prisma.product.findMany({
                where : {stock: {gt: 0}},
                orderBy: {originalPrice: 'desc'},
                //'take:6' means it can only take 6 products only
                take : 6
            })
    ...
    let sentCount = 0;
    // send email in batches of 10 to avoid overwhelming mail server; 'u' = user in below
    const batchSize = 10;
    for(let i =0; i < users.length; i += batchSize){
        ...
        await step.run(`send-offers-batch-${i}`, async () => {
            for(const u of batch){
               await sendEmail({
                to:u.email,
                subject: `Fresh farm produce for you`,
                body:`<div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: auto; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
...
const autoAssignRidder = inngest.createFunction({
  ...
  triggers :[{event : 'order/placed'}]
    ...
    const result = await step.run('assign-rider', async () => {
        const order = await prisma.order.findUnique({
            ...
            //find an active rider not currently delivering 'o'===order
...
export const functions = [checkLowStock, sendMonthlyOffers, autoAssignRidder];
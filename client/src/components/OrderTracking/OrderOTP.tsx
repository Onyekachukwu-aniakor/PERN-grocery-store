...
export default function OrderOTP({ order }: { order: any }) {
    ...
    return (
        <div className="bg-linear-to-r from-app-green to-app-green-light rounded-2xl p-4 text-white">
            ...
            <div className="flex gap-2 mt-2">
                {order.deliveryOtp.split("").map((digit: string, i: number) => (
                    <div key={i} className="w-10 h-10 rounded-xl bg-white/15 flex-center text-2xl font-mono font-bold tracking-wider">
                        {digit}
                    </div>
                ))}
            </div>
